...
def compute_fingerprints(molecules, method, actives, decoys):
    if method == "pseudo_usr":
        ...
        return {mol: generate_nd_molecule_fingerprint_with_flip(mol, features=None, scaling_method='matrix') for mol in molecules}
        ...
        return {mol: generate_nd_molecule_fingerprint_with_flip(mol, features=PSEUDO_ELECTROSHAPE_FEATURES, scaling_method='matrix') for mol in molecules}
...
def get_pseudo_usrcat_fingerprint(mol):
    ...
    mol_3d_pca, _ = compute_pca_using_covariance_with_flip(mol_3d)
    ...
    if os.path.isdir(folder_path):
        ...
        for query_mol in actives:
            ...
            if method == 'pseudo_usr_cat':
                ...
                y_scores = [compute_similarity_score_with_flip(query_fp, fingerprints[mol]) for mol in other_mols]
...
if __name__ == "__main__":
    ...
    output_filename = f"{os.getcwd()}/dude_pseudo_results_chirality_with_flip.txt"
