...
def test_pca(n_dim):
    ...
    _, transformed_data, eigenvectors, _ = compute_pca_using_covariance(original_data)
    ...
    # Test eigenvectors
    for i in range(n_dim):
        matches = [np.isclose(np.abs(np.dot(eigenvectors[:, i], basis_vector)), 1 , atol=0.3) for basis_vector in np.eye(n_dim)]
        assert any(matches)
