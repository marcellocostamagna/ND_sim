...
def compute_6D_similarity(query, target):
    ...
    data = np.hstack((query['coordinates'], np.array(query['protons']).reshape(-1, 1), np.array(query['formal_charges']).reshape(-1, 1), np.array(query['delta_neutrons']).reshape(-1, 1)))
    data1 = np.hstack((target['coordinates'], np.array(target['protons']).reshape(-1, 1), np.array(target['formal_charges']).reshape(-1, 1), np.array(target['delta_neutrons']).reshape(-1, 1)))
