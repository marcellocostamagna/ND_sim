...
def save_to_sd(query_mol, target_mol, folder_name, method, base_file_name="similar_molecules"):
    ...
    index = 1
    while os.path.exists(f"similar_molecules/{base_file_name}{index}.sdf"):
        index += 1
    file_path = f"similar_molecules/{base_file_name}{index}.sdf"

    w = Chem.SDWriter(file_path)
    for mol in [query_mol, target_mol]:
        mol.SetProp("Folder", folder_name)
        mol.SetProp("Method", method)
        mol.SetProp("Score", "1.0")
        w.write(mol)
    w.close()
    ...
    if os.path.isdir(folder_path):
        ...
        for query_mol in actives:
            ...
            for mol, score in zip(other_mols, y_scores):
                if math.isclose(score, 1.0, rel_tol=1e-5):  # using a relative tolerance of 1e-4
