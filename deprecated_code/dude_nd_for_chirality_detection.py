...
def process_folder(args):
    ...
    if os.path.isdir(folder_path):
        ...
        for query_mol in actives:
            ...
            for mol in other_mols:
                if method == 'pseudo_usr_cat':
                    ...
                    score, chirality_signal = compute_similarity_score(query_fp, fingerprints[mol])
                    if chirality_signal == 'Possible_Chirality!':
                        chirality_count += 1
        ...
        results[folder]['chirality_count'] = chirality_count
        
       # Printing results
        print(f"\nAverage Enrichment Factors for folder {folder}:")
        for key, value in avg_folder_enrichments.items():
            if key == 'chirality_count':
                continue  # Skip chirality for now
            else:
                print(f"Enrichment Factor at {key*100}%: {value}")

        # Print chirality occurrences after enrichment factors
        if avg_folder_enrichments.get('chirality_count', 0) > 0:
            print(f"    Chirality Occurrences: {avg_folder_enrichments['chirality_count']}")
        else:
            print(f"    No Chirality Detected")
...
if __name__ == "__main__":
    ...
    methods = ['pseudo_usr', 'pseudo_electroshape']
    ...
    # Print the overall average enrichment factors (without chirality)
    print("\nAverage Enrichment Factors for Different Methods:")
    for method, folders in overall_results.items():
        print(f"\nResults for {method}:")
        for folder, folder_results in folders.items():
            print(f"  Folder: {folder}")
            for key, value in folder_results.items():
                if key == 'chirality_count':
                    continue  # Skip chirality
                else:
                    print(f"    Enrichment Factor at {key*100}%: {value}")

    output_filename = f"{os.getcwd()}/dude_nd_chirality_detection.txt"
